...
export function EditStartupClient() {
    ...
    return (
      <div className="text-center py-24">
        <p className="text-slate-500">
          {startup ? t('edit.noAccess') : t('edit.notFound')}
        </p>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        >
          <Trash2 className="h-3.5 w-3.5" /> {t('edit.delete')}
        </button>
      </div>

      <StartupForm initial={startup} />

      <Modal open={confirmOpen} onClose={() => setConfirmOpen(false)} title={t('edit.deleteTitle')}>
        <div className="space-y-4">
          <p className="text-subhead text-slate-600 leading-relaxed">
            {t.rich('edit.deleteConfirm', {
              title: startup.title,
          ...
          <div className="flex gap-3 justify-end">
            ...
            <Button variant="danger" loading={removing} onClick={() => remove()}>
              <Trash2 className="h-4 w-4" /> {t('edit.delete')}
            </Button>
          </div>
        </div>
      </Modal>
