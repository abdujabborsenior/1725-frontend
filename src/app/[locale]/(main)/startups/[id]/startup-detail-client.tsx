...
export function StartupDetailClient({ initialStartup }: { initialStartup: Startup | null }) {
  const t = useTranslations('startupDetail');
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="relative z-10 px-5 md:px-8 pb-6">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 text-footnote text-slate-500">
            {startup.category && (
              <span className="inline-flex items-center gap-1.5 text-slate-600">
                <Tag className="h-3.5 w-3.5 text-accent-600" /> {catLabel(startup.category)}
              </span>
            )}
            {startup.teamName && (
            ...
            <span className="inline-flex items-center gap-1.5">
              ...
              {t('views', { count: startup.viewCount, n: fmt(startup.viewCount) })}
            </span>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="tappable inline-flex h-9 items-center gap-1.5 rounded-full bg-fill-tertiary px-3.5 text-subhead font-medium text-slate-600 transition-colors hover:bg-accent-50 hover:text-accent-700"
            >
              <Flag className="h-[17px] w-[17px]" /> {t('report')}
            </button>
            {isOwner && (
              <Link
                ...
                className="tappable inline-flex h-9 items-center gap-1.5 rounded-full bg-fill-tertiary px-3.5 text-subhead font-medium text-slate-600 transition-colors hover:bg-accent-50 hover:text-accent-700"
              >
                <PencilLine className="h-[17px] w-[17px]" /> {t('edit')}
              </Link>
            )}
          </div>
          <ReportDialog
          ...
          {orderedPlatforms.length > 0 ? (
            ...
            <div className="mt-6 inline-flex items-center gap-2 rounded-ios-md bg-amber-50 px-4 py-2.5 text-subhead text-amber-700">
              <Rocket className="h-[17px] w-[17px]" /> {t('ideaStage')}
            </div>
          )}
        </div>
      </div>
      ...
      <section className="space-y-3">
        <h2 className="text-title-2 font-semibold text-brand-900">{t('about')}</h2>
      ...
      {orderedPlatforms.length > 0 && (
        <section className="space-y-3">
          <h2 className="text-title-2 font-semibold text-brand-900">{t('allLinks')}</h2>
      ...
      {related && related.length > 0 && (
        <section className="space-y-3">
          <h2 className="text-title-2 font-semibold text-brand-900">{t('related')}</h2>
