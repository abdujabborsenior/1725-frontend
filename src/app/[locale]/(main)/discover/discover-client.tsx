...
/** Natija turlari — yorliqlar lug'atda (`discover.tabs.<tab>`) */
const TABS: Tab[] = ['all', 'people', 'groups'];
    ...
    <div className="mx-auto max-w-4xl space-y-7">
      ...
      {searching ? (
        <div className="animate-fade-in space-y-6">
          ...
          {(tab === 'all' || tab === 'people') && (
            <section>
              <h2 className="ios-section-header">{t('tabs.people')}</h2>
              <div className="ios-list" style={{ ['--row-inset' as string]: '3.75rem' }}>
                {peopleArr.length > 0 ? (
                  peopleArr.map((u) => <UserListItem key={u.id} user={u} />)
                ) : !isFetching ? (
                  <p className="py-8 text-center text-subhead text-slate-500">{t('noPeople')}</p>
                ) : (
          ...
          {(tab === 'all' || tab === 'groups') && (
            <section>
              <h2 className="ios-section-header">{t('tabs.groups')}</h2>
              {groupsArr.length > 0 ? (
        ...
        <div className="space-y-7">
          ...
          {recent.length > 0 && (
            <section>
              <div className="mb-3 flex items-center justify-between">
                <h2 className="ios-section-header pb-0">{t('recent')}</h2>
                <button
                ...
                {recent.map((r) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      aria-label={t('removeRecent', { term: r })}
          ...
          {/* Suggestions */}
          <section>
            <h2 className="ios-section-header">{t('suggestions')}</h2>
            <div className="ios-list" style={{ ['--row-inset' as string]: '3.75rem' }}>
          ...
          {/* Groups */}
          <section>
            <h2 className="ios-section-header">{t('communityGroups')}</h2>
            {groups && groups.length > 0 ? (
              ...
              <EmptyState icon={<Users />} title={t('noCommunityGroups')} />
            )}
          </section>
        </div>
      )}
