...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {/* Profile card */}
      <div className="rounded-ios-2xl bg-white p-6 md:p-8 shadow-card">
        <div className="flex items-start gap-5">
          <Avatar src={user.avatarUrl} name={user.fullName} size={80} />
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 flex-wrap">
              <h2 className="text-title-2 font-semibold text-brand-900">{user.fullName}</h2>
              <span className={`rounded-full px-2.5 py-1 text-caption-1 font-medium ${ROLE_BADGE[user.role]}`}>
                {ROLE_LABEL[user.role] ?? user.role}
              </span>
              {user.isFounder && <FounderBadge />}
              {user.isEmailVerified && (
                <span className="flex items-center gap-1 text-caption-1 text-accent-700 font-semibold">
            ...
            <div className="mt-2 flex items-center gap-5">
              <span><span className="font-semibold text-brand-900">{(user.followerCount ?? 0).toLocaleString('uz')}</span> <span className="text-subhead text-slate-500">obunachi</span></span>
              <span><span className="font-semibold text-brand-900">{(user.followingCount ?? 0).toLocaleString('uz')}</span> <span className="text-subhead text-slate-500">obuna</span></span>
            </div>
            {user.username && (
          ...
          <div className="mt-6 pt-5 border-t border-slate-200 grid grid-cols-2 md:grid-cols-3 gap-4">
            {details.map(({ icon: Icon, label, value }) => (
              <div key={label} className="flex flex-col gap-0.5">
                <span className="text-caption-2 text-slate-400 uppercase tracking-wide flex items-center gap-1">
      ...
      {(bookmarks?.data?.length ?? 0) > 0 && (
        <div>
          <h2 className="mb-4 flex items-center gap-2 text-title-3 font-semibold text-brand-900">
      ...
      <div>
        ...
        {myProblems === undefined ? (
          ...
          <div className="rounded-ios-2xl bg-white p-10 text-center">
          ...
          <div className="space-y-3">
            {(myProblems?.data ?? []).map((p) => (
              <Link key={p.id} href={`/problems/${p.id}`}>
                <div className="rounded-ios-lg bg-white p-4 hover:shadow-card transition-all group">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                      <ProblemStatusBadge status={p.status} className="px-2 py-0.5 text-caption-2" />
                      <div className="flex items-center gap-2 text-caption-2 text-slate-400">
              ...
              <Link key={s.id} href={s.problem ? `/problems/${s.problem.id}` : '#'}>
                <div className="rounded-ios-lg bg-white p-4 hover:shadow-card transition-all group">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="inline-flex flex-shrink-0 items-center gap-1 rounded-full bg-accent-50 px-2 py-0.5 text-caption-2 font-medium text-accent-700">
      ...
      <Modal open={showDeleteConfirm} onClose={() => setShowDeleteConfirm(false)} className="max-w-sm">
        ...
        <h3 className="mb-2 text-center text-title-3 font-semibold text-brand-900">Hisobni o&apos;chirish</h3>
