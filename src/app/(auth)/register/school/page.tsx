...
import { z } from 'zod';
import {
  User, Mail, Lock, Eye, EyeOff, School,
  ArrowRight, ArrowLeft, Zap, CheckCircle2, MapPin,
} from 'lucide-react';
...
const schema = z.object({
  ...
  age:             z.coerce.number().min(6,'6 yoshdan').max(18,'18 yoshgacha'),
  region:          z.string().min(1, 'Viloyat tanlang'),
  district:        z.string().min(2, 'Tuman nomini kiriting'),
  school:          z.string().min(2, 'Maktab nomini kiriting'),
  grade:           z.coerce.number().min(1).max(11),
...
const STEPS = [
  ...
  { title: 'Maktab', subtitle: 'Ta\'lim ma\'lumotlari' },
...
export default function RegisterSchoolPage() {
  ...
  async function onSubmit(data: FormData) {
    try {
      await authApi.registerSchool({
        ...
        district: data.district, school: data.school, grade: data.grade,
  ...
  const gradeOptions = Array.from({ length: 11 }, (_, i) => ({
    value: String(i + 1), label: `${i + 1}-sinf`,
  }));
        ...
        <div className="glass-strong rounded-3xl p-8 shadow-card">
          ...
          <div className="flex justify-center mb-6">
            <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-neon-green/10 border border-neon-green/30">
              <School className="h-4 w-4 text-neon-green" />
              <span className="text-sm font-semibold text-neon-green">Maktab o&apos;quvchisi</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
            {step === 0 && (
              <>
                <Input label="To'liq ism" placeholder="Ism Familiya"
                  ...
                  {...register('fullName')} />
                <Input label="Yosh" type="number" placeholder="14" min={6} max={18}
            ...
            {step === 1 && (
              <>
                ...
                <Input label="Tuman" placeholder="Tuman nomi"
                  icon={<MapPin className="h-4 w-4" />} error={errors.district?.message}
                  {...register('district')} />
                <Input label="Maktab" placeholder="Maktab nomi yoki raqami"
                  icon={<School className="h-4 w-4" />} error={errors.school?.message}
                  {...register('school')} />
                <Select label="Sinf" options={[{ value: '', label: 'Sinf tanlang' }, ...gradeOptions]}
                  error={errors.grade?.message} {...register('grade')} />
