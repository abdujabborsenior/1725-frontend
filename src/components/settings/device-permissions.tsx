...
import { Camera, Mic, Images, Loader2, ShieldCheck } from 'lucide-react';
import {
    ...
    <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-4">
      ...
      <div className="divide-y divide-slate-100">
        {rows.map(({ key, icon: Icon, title, desc, state }) => {
          ...
          return (
            <div key={key} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-surface-soft text-slate-500">
              ...
              <span className={cn('shrink-0 rounded-full border px-2.5 py-1 text-[11px] font-semibold', meta.cls)}>
                {meta.label}
              </span>
              {canRequest && (
                <button
                  ...
                  disabled={busy !== null}
                  className="shrink-0 rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-brand-900 transition-colors hover:border-slate-300 hover:bg-slate-50 disabled:opacity-60"
                >
                  {busy === key ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "So'rash"}
                </button>
              )}
            </div>
          );
        ...
        <div className="flex items-center gap-3 py-3 last:pb-0">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-semibold text-brand-900">Galereya</p>
            <p className="text-xs text-slate-500">
              Rasm yoki video tanlaganingizda qurilmaning o&apos;zi ruxsat so&apos;raydi
            </p>
          </div>
          <span className="shrink-0 rounded-full border border-slate-200 bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-600">
            Avtomatik
          </span>
        </div>
      </div>
