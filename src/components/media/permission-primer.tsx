...
import { Camera, Mic, X, Loader2 } from 'lucide-react';
import {
...
export function MediaPermissionPrimer() {
  ...
  return (
    <div className="fixed inset-x-4 z-50 bottom-[calc(4.75rem+env(safe-area-inset-bottom))] sm:inset-x-auto sm:right-6 sm:bottom-6 sm:w-[380px] animate-slide-up">
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-modal">
        <div className="flex items-start gap-3.5">
          ...
          <button
            onClick={later}
            aria-label="Yopish"
            className="-mr-1 -mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-slate-100 hover:text-brand-900"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
        <div className="mt-4 flex items-center gap-2">
          <button
            onClick={allow}
            disabled={busy}
            className="btn-lift inline-flex h-10 flex-1 items-center justify-center gap-2 rounded-xl bg-accent-700 text-sm font-semibold text-white transition-colors hover:bg-accent-800 disabled:opacity-60"
          >
            {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Ruxsat berish'}
          </button>
          <button
            onClick={later}
            className="inline-flex h-10 items-center justify-center rounded-xl px-4 text-sm font-semibold text-slate-500 transition-colors hover:bg-slate-100 hover:text-brand-900"
          >
            Keyinroq
          </button>
        </div>
